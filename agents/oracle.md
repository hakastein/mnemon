...
1. Pull the assigned material FULLY into your context:
   ...
   - **A GitHub repo** → fetch only what the question needs. A few known files → WebFetch their `raw.githubusercontent.com` URLs whole. A heavy repo or broad coverage → clone the *whole* repo and read it locally instead of WebFetching source page by page: `git clone --depth 1 <url> <path>` (or `gh repo clone`), then Glob + Read what matters. Pick the path by context — a scratch dir (`/tmp/<name>`) for a throwaway look, `.refs/<name>` (or the project's reference dir) for a durable reference. If the caller gave you a clone path (and maybe a code-review-graph reading list over it), use it; if a sandbox blocks your reads there, say so and propose an in-tree path. If you're being asked to WebFetch broadly across an unfamiliar repo, stop and tell the caller it's worth cloning + graphing the repo first rather than fetching it page by page.
